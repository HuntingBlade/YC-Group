...
public class HttpSvcUtils {
	...
	static Logger logger = Logger.getLogger(HttpSvcUtils.class
			.getName());
	 ...
	 * @throws IOException
	 */
	public static JSONObject doGetSvc(String url) {
		if (url == null) {
			return null;
		}
		try {
	...
	public static String doGet(String url,String charset) {
		...
		try {
			...
			if(!SyStringUtils.isBlank(charset)){

				byte[] b=get.getResponseBody();
				if(b!=null){

					response=	EncodingUtil.getString(b, charset);
				}
			}else{
	...
	public static JSONObject doPost(String url, JSONObject jsonParam,String accessToken) {
...
//		}
		...
		try {
			...
			if(jsonParam!=null){
				RequestEntity r=new StringRequestEntity(jsonParam.toString(),"application/json","charset=UTF-8");
		...
		return JSONObject.parseObject(strRequest);
